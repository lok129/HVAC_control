...
class Chiller_Model:
    def __init__(self,CLS,action):
        self.CLS = CLS[0]
        self.T_outdoor = CLS[1]   # CLS为状态 由三变量组成【当前冷负荷，户外湿球温度，预测冷负荷】
        self.T_cwr = CLS[1]
        self.T_chws =  action
        self.T_chwr = self.get_variable()[1]
        self.CLC = self.get_variable()[0]
        self.coeff = np.load("coeff_Date.npy") #这便是嵌入冷却塔精确数据
        self.step_len = 0.5
        self.F_nominal = 581
        self.F_cw = 252
        self.F_tower = 569
        # self.F_chw =   131  #冷冻水流量
    ...
    def get_P(self):
            ...
            while True:
                #+++++++++++++++++冷水机模型++++++++++++++++++++++
                # 先是计算T_chws以及T_chwr,其中T_chws已经指定出水的温度已经指定好了。先计算公式3
                # self.T_chwr = self.T_chws + self.CLc / ((self.c_p * self.F_chw * self.density_water) / (3600))
                T_chwr = self.T_chwr
                ...
                #+++++++++++++++++冷却塔模型++++++++++++++++++++++++
                # 计算出Error值
                minerror,T_cwr = self.caculate_Error()
                self.T_cwr = T_cwr
            ...
            P_pump = 55
            P_tower = 16.5
            return P_chiller,P_tower,P_pump,T_cwr,self.T_cws
    ...
    def caculate_Error(self):
        FRair = 50/50
        FRwater = ((50 / 50)*self.F_nominal)/self.F_tower
        T_wb = self.T_outdoor
        T_cwr = T_wb
        T_cws = self.T_cws
        ERRORs = []

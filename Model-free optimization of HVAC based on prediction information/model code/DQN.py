...
class choose_tchws:
    ...
    def select_action(self, state):#
        ...
        if rand_num > EPS_Ratio: # greedy
                ...
                index = torch.argmax(list[0][0:10])
                return index
        else:
                state = torch.unsqueeze(torch.FloatTensor(state), 0).to(device)
                return torch.tensor([[random.randrange(0,10)]], device=device, dtype=torch.long).item()
...
class DQN:
    ...
    def train(self):
        Agent = choose_tchws(3,10,self.action)
        R_list = []
        num = 0
        for episode in range(episodes):
            ...
            Es_sheet = book.add_sheet('Es' + str(episode), cell_overwrite_ok=True)
            comfort_sheet = book.add_sheet('comfort' + str(episode), cell_overwrite_ok=True)
            ...
            while True:
                ...
                T_chws = self.action[action_index]

                S_,CLc,Done,P_chiller,R,Es,comfort,T_chwr,T_outdoor,P_chiller_total= env.step(T_chws)

                CLs_data_sheet.write(row, 0, str(CLs))
                CLc_data_sheet.write(row, 0, str(CLc))
                r_sheet.write(row, 0, str(R[0]))
                P_chiller_sheet.write(row, 0, str(P_chiller[0]))
                Es_sheet.write(row, 0, str(Es))
                comfort_sheet.write(row, 0, str(comfort[0]))
                T_chws_sheet.write(row, 0, str(T_chws[0]))
                T_chwr_sheet.write(row, 0, str(T_chwr[0]))
                T_outdoor_sheet.write(row, 0, str(T_outdoor))
                P_chiller_total_sheet.write(row,0,str(P_chiller_total))
